...
- **Sankhyas Score** (`js/insights.js`): one 0–100 score per company from five pillars. Each pillar is a percentile rank against all listed companies:
  - Quality: ROCE, ROE, margins, 5-year ROCE
  - Growth: 3-year and latest-quarter sales and profit growth
  - Value: P/E, P/B, earnings and dividend yield
  - Momentum: 6-month and 1-year returns, price vs 200 DMA
  - Safety: debt, interest cover, pledges, red flags

  The score and the rank within the sector are free; the pillar breakdown is Pro. All of them can be screened (`Sankhyas Score > 75`); see the **Sankhyas Top Scorers** preset.
- **Smart money** (`#/deals`, `scripts/fetch_deals.py`): NSE's daily bulk and block deal files, kept in a rolling 400-day store. It shows net buying and selling by company, every deal with client, side, quantity, price and value, and insider-trading (Reg 7) and takeover (SAST) disclosures. For those disclosures, the buy/sell/pledge direction is read from the filing PDF. Screenable as `Bulk/block net buy 3M`.
- **Order wins** (`#/orders`): every "bagging/receiving of orders" filing. The order value, customer and description are read from the PDF, and foreign-currency values are converted to ₹ Cr. The page shows totals, the biggest orders, order value as % of sales, and a theme filter. Screenable as `Order wins 12M`; see the **Order Book Momentum** preset. Company pages show both in Sankhyas Insights.
- **Research PDF** (`#/report/<SYMBOL>`, Pro): a branded 2-page report with key ratios, Sankhyas Score, 6 years of financials, recent quarters, red flags, guidance, order wins and the latest concall summary. It uses the browser's "Save as PDF" (print styles included).
